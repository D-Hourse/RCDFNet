...
def tj4d_eval(gt_annos,
    ...
    if len(current_classes) > 1:
        ...
        results_dir = Path(
            getattr(config, 'tj4d_results_dir', '')
            or os.getenv('TJ4D_RESULTS_DIR', 'work_dirs/cpf_results_tj4d'))
        results_dir.mkdir(parents=True, exist_ok=True)
        results_file = results_dir / 'tj4d_allresult2.txt'

        if config.myepoch == 1:
            with results_file.open('w', encoding='utf-8') as file:
                file.write('epoch_' + str(config.myepoch) + '\n')
                file.write(result + '\n')
                config.myepoch = config.myepoch + 1
        else:
