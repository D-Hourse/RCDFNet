...
from .image_vis import (draw_camera_bbox3d_on_img, draw_depth_bbox3d_on_img,
                        draw_lidar_bbox3d_on_img, draw_lidar_bbox3d_on_img_my)
...
def show_multi_modality_result(img,
                               ...
                               img_metas=None,
                               show=True,
                               gt_bbox_color=(0, 102, 255),
                               pred_bbox_color=(241, 0, 72)):
    ...
    if show:
        ...
        if pred_bboxes.corners is not None:
            show_img = draw_bbox(
                pred_bboxes,
                show_img,
                proj_mat,
...
def show_multi_modality_result_my(img,
                               gt_bboxes,
                               pred_bboxes,
                               pred_labels,
                               gt_labels,
    ...
    Args:
        ...
        gt_bbox_color (str or tuple(int)): Color of bbox lines.
           The tuple of color should be in BGR order. Default: (255, 102, 61)
        pred_bbox_color (str or tuple(int)): Color of bbox lines.
           The tuple of color should be in BGR order. Default: [(241, 0, 72), (125, 0, 255), (0, 255, 255)]
    """
    if pred_bbox_color is None:
        pred_bbox_color = [(241, 0, 72), (125, 0, 255), (255, 255, 0)]
        # pred_bbox_color = [(255, 255, 0), (125, 0, 255), (255, 255, 0)]
    if box_mode == 'depth':
        draw_bbox = draw_depth_bbox3d_on_img
    elif box_mode == 'lidar':
        draw_bbox = draw_lidar_bbox3d_on_img
    ...
    draw_bbox_my = draw_lidar_bbox3d_on_img_my
    ...
    if gt_bboxes is not None:
        if gt_bboxes.__len__() == 0:
            ...
            if gt_labels is None:
                ...
                gt_img = draw_bbox_my(
                    gt_bboxes, gt_labels, img, proj_mat, img_metas, color=pred_bbox_color)
                mmcv.imwrite(gt_img, osp.join(result_path, f'{filename}_gt.png'))
    ...
    if pred_bboxes.corners is not None:
        if pred_bboxes.__len__() == 0:
            ...
            if pred_labels is None:
                pred_img = draw_bbox(
                    pred_bboxes, img, proj_mat, img_metas, color=pred_bbox_color[0])
                mmcv.imwrite(pred_img, osp.join(result_path, f'{filename}_pred.png'))
            else:
                pred_img = draw_bbox_my(
                    pred_bboxes, pred_labels, img, proj_mat, img_metas, color=pred_bbox_color)
                mmcv.imwrite(pred_img, osp.join(result_path, f'{filename}_pred.png'))
