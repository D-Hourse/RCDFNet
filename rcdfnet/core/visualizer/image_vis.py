...
def plot_rect3d_on_img_my(img,
    ...
    """
    if color is None:
        color = [(241, 0, 72), (125, 0, 255), (0, 255, 255)]
    line_indices = ((0, 1), (0, 3), (0, 4), (1, 2), (1, 5), (3, 2), (3, 7),
                    (4, 5), (4, 7), (2, 6), (5, 6), (6, 7))
    for i in range(num_rects):
        if num_labels[i] == -1:
        ...
        for start, end in line_indices:
            cv2.line(img, (corners[start, 0], corners[start, 1]),
                     (corners[end, 0], corners[end, 1]), color[num_labels[i]], thickness,
                     cv2.LINE_AA)
...
def draw_lidar_bbox3d_on_img_my(bboxes3d,
                             labels3d,
                             raw_img,
                             lidar2img_rt,
                             img_metas,
    ...
    Args:
        ...
        lidar2img_rt (numpy.array, shape=[4, 4]): The projection matrix
            according to the camera intrinsic parameters.
        img_metas (dict): Useless here.
        color (list[tuple[int]]): The color to draw bboxes. Default: [(0, 255, 0), (x, x, x), (x, x, x)].
        thickness (int, optional): The thickness of bboxes. Default: 1.
    """
    if color is None:
        color = [(241, 0, 72), (125, 0, 255), (0, 170, 255)]
    img = raw_img.copy()
    corners_3d = bboxes3d.corners
    num_bbox = corners_3d.shape[0]
    pts_4d = np.concatenate(
    ...
    return plot_rect3d_on_img_my(img, num_bbox, labels3d, imgfov_pts_2d, color, thickness)
...
def draw_depth_bbox3d_on_img(bboxes3d,
    ...
    Args:
        ...
        thickness (int, optional): The thickness of bboxes. Default: 1.
    """
    from rcdfnet.core.bbox import points_cam2img
    from rcdfnet.models import apply_3d_transformation
