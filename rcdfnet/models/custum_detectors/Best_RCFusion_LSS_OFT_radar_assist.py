############modify Best_RCFusion_LSS_OFT_radar_assist##########
import torch
...
from .OFTNet_new import OftNet
from .LSSNet_new import LSSNet
...
class Radar_assist_attention(nn.Module):
    def __init__(self, in_channels, out_channels, width1, width2):
        super(Radar_assist_attention, self).__init__()
        self.radar_assist_attention = nn.Sequential(
            nn.Conv2d(in_channels, in_channels//3, kernel_size=3, stride=1, padding=1, padding_mode='reflect'),
            nn.BatchNorm2d(in_channels//3),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels//3, out_channels, kernel_size=1, stride=1, padding=0)
        )
        self.width1 = width1
        self.width2 = width2
        self.map = nn.Linear(width1, width2)
    def forward(self, x):
        out = self.radar_assist_attention(x)
        out_OFT = out.sigmoid().permute(0, 3, 1, 2).contiguous()
        out = out.permute(0, 1, 3, 2).contiguous()
        batch, depth, height, width = out.shape
        out_LSS = self.map(out.view(batch, depth*height, width).contiguous()).view(batch, depth, height, self.width2).permute(0, 3, 2, 1)
        out_LSS = out_LSS.sigmoid().permute(0, 2, 3, 1)
        return out_OFT, out_LSS

@DETECTORS.register_module()
class Best_RCFusion_LSS_OFT_radar_assist(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
        ...
        """
        super(Best_RCFusion_LSS_OFT_radar_assist, self).__init__(**kwargs)
        self.rc_fusion = rc_fusion
        self.lift = camera_stream
        self.se = se
        self.radar_att = Radar_assist_attention(384, int((grid_z_max-grid_z_min)/grid_res), int((y_bound[1]-y_bound[0])/y_bound[2]), int(final_dim[1]/downsample_factor))
        ...
        if camera_stream:
            ...
            self.lss_BEVencoder = LSSNet(x_bound=x_bound, y_bound=y_bound, z_bound=z_bound,
                                         ...
                                         depth_net_conf=dict(in_channels=depth_net_in_channels,mid_channels=depth_net_mid_channels),
                                         depth_channels=self.depth_channels, return_depth=return_depth)
            self.bev_attention = BEVFuser(bev1_dims=256, bev2_dims=256, embed_dims=256, num_layers=4, num_heads=4, bev_shape=(160, 160))
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        OFT_radar_att, LSS_radar_att = self.radar_att(pts_feats[0])
        # -------------提升的过程写在此处--------------------
        if self.lift:
            ###判断batch一致
            ...
            if self.return_depth:
                ...
                img_bev_feat_L_scale = self.oft_BEVencoder(img_feats, OFT_radar_att, calib)
                img_bev_feat_S_scale, depth = self.lss_BEVencoder(img_feats, LSS_radar_att, calib)
