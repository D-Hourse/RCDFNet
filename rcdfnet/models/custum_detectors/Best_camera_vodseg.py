...
from .LSSNet_new import LSSNet, IFSO
...
class Best_camera_segsup(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True, segmentation=False,
        ...
        super(Best_camera_segsup, self).__init__(**kwargs)
        ...
        if camera_stream:
            ...
            self.lss_BEVencoder = LSSNet(x_bound=x_bound, y_bound=y_bound, z_bound=z_bound,
                                         ...
                                         depth_net_conf=dict(in_channels=depth_net_in_channels,
                                                             ...
                                                             camera_aware=True),
                                         depth_channels=self.depth_channels, return_depth=return_depth, return_seg=self.segmentation)
        ...
        if self.lift:
            ...
            if self.return_depth:
                if self.segmentation:
                    img_bev_feat_S_scale, depth, seg = self.lss_BEVencoder(img_feats, calib)
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False):
        ...
        feature_dict = self.extract_feat(
            points, img_inputs=img, img_metas=img_metas)
    ...
    def depth_gt_transform(self, depth, img_metas):
        depth_map_return = []
    ...
    def forward_train(self,
        ...
        if self.return_depth:
            depth_gt = self.depth_gt_transform(depth, img_metas)
