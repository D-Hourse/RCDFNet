...
class Best_camera_dualbev_bevconv_ida(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
                 ...
                 grid_z_max=2.76, se=False, imc=256,radc=384, return_mask=False,loss_depth_weight=None, loss_seg_weight=None,
                 **kwargs):
        ...
        super(Best_camera_dualbev_bevconv_ida, self).__init__(**kwargs)
    ...
    def extract_feat(self, points, img_inputs, img_metas):
        ...
        if self.lift:
            ...
            if self.return_mask:
                img_bev_feat, depth, bev_mask = self.BEVencoder(img_feats, img_metas_temp)
            else:
            ...
            if pts_feats is None:
                ...
                if self.rc_fusion == "concat":  # 大小shape不一致，用插值
                    ...
                    # print("draw done")
        return_dict = dict(img_feats=img_feats, pts_feats=pts_feats, depth_preds=depth)
        if self.return_mask:
            return_dict.update(bev_mask=bev_mask)
    ...
    def forward_train(self,
        ...
        if pts_feats:
            ...
            if self.return_mask:
                losses_mask_ce, losses_dice = self.BEVencoder.prob.get_bev_mask_loss(gt_bev_mask, feature_dict['bev_mask'])
