...
from .other_modules import PointFusion_region
...
class Best_RCFusion_LSS_OFT_pointFusion_region(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
                 ...
                 grid_z_max=2.76, se=False, imc=256, radc=384, voxel_size=[0,0,0], x_bound=[0, 69.12, 0.32],y_bound=[-39.68, 39.68, 0.32],
        ...
        """
        point_fusion_config = kwargs.get('point_fusion_region', None)
        kwargs.pop('point_fusion_region')
        super(Best_RCFusion_LSS_OFT_pointFusion_region, self).__init__(**kwargs)
        ...
        if camera_stream:
            ...
            if point_fusion_config is not None:
                self.point_Fusion = PointFusion_region(**point_fusion_config)
    ...
    def isfusion(self, pts, pillars, pillars_num_points, pillar_coors, pts_feats, img_feats, img_metas, batch_size):
        ...
        lidar2img = []
        for i in img_metas:
            lidar2img.append(i['lidar2img'])
        kwargs.update(dict(lidar2img=lidar2img))

        x = self.point_Fusion(img_feats, pts_feats, batch_size, **kwargs)
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        pts_feats = self.extract_pts_feat(points, img_feats[1], img_metas)  # 点云特征ortho  ###(B, 384, 124, 108)
        # -------------提升的过程写在此处--------------------
        if self.lift:
            ### 判断batch一致
            ...
            if pts_feats is None:
                ...
                if self.rc_fusion == "concat":  # 大小shape不一致，用插值
                    ...
                    if img_bev_feat.shape[2:] != pts_feats.shape[2:]:
                        img_bev_feat = F.interpolate(img_bev_feat, pts_feats.shape[2:], mode='bilinear',
                                                     align_corners=True)
                    # pts_feats = [self.reduc_radar_conv(pts_feats[0])]
                    pts_feats = [self.cross_attention(img_bev_feat, pts_feats)]
