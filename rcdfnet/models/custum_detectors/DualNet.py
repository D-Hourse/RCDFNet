...
class DualNet(LSSNet):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, final_dim, collapse_z,
        ...
        super(DualNet, self).__init__(x_bound, y_bound, z_bound, d_bound, final_dim,
                 downsample_factor, depth_net_conf, depth_channels, return_depth, output_channels)
        ...
        self.bev_h = int((self.grid_config['x'][1] - self.grid_config['x'][0]) // self.grid_config['x'][2])
        self.bev_w = int((self.grid_config['y'][1] - self.grid_config['y'][0]) // self.grid_config['y'][2])
        ...
        self.register_buffer('voxel_hp', self.get_reference_points_3d(self.bev_h, self.bev_w, num_points_in_pillar=self.num_height))

    def get_reference_points_3d(self, H, W, Z=8, num_points_in_pillar=13, device='cuda', dtype=torch.float):
    ...
    def forward(self, features, calib):
        ...
        #             self.voxel_size).int()
        # lss_feat = voxel_pooling(geom_xyz, img_feat_with_depth.contiguous(),
        #                             self.voxel_num.cuda())

        #------------------------------Height transform bev-----------------------
        # 对depth和context增加维度适应ht
        ...
        channel_feat = ht_feat
        if self.return_mask:
            mask = torch.zeros((B, self.bev_h, self.bev_w), device=ht_feat.device).to(ht_feat.dtype)
            bev_pos = self.positional_encoding(mask).to(ht_feat.dtype)
