...
from ..module.EMCAD import EMCAD_FPN
from .OFTNet import OftNet_EMCAD
...
class Best_RCFusion_EMCADDecoder_LSS_OFT4(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
        ...
        super(Best_RCFusion_EMCADDecoder_LSS_OFT4, self).__init__(**kwargs)
        ...
        ####-----------EMCAD replace FPN-------
        self.decoder = EMCAD_FPN(**img_seg_decoder_config)

        ####-----------LSS分支初始化--------
        if camera_stream:
            self.oft_BEVencoder = OftNet_EMCAD(img_bev_harf=img_bev_harf,grid_size=grid_size, grid_offset=grid_offset, grid_res=grid_res, grid_z_min=grid_z_min, grid_z_max=grid_z_max)
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if self.decoder:
            img_feats = self.decoder(img_feats)
