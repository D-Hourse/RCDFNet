...
class Best_RCFusion_OFT3_segsup_Focolloss(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True, segmentation=False,
        ...
        super(Best_RCFusion_OFT3_segsup_Focolloss, self).__init__(**kwargs)
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if self.segmentation:
            ...
            for feat in img_feats:
                feats_temp = self.IFSO(feat, img_metas)
                img_temp = feats_temp[0]
                seg_temp = feats_temp[1].softmax(dim=1)
                img_mask = torch.sum(seg_temp[:, 1:4, :, :], dim=1) >= self.seg_threshold
                filter_feat = img_mask.unsqueeze(dim=1) * img_temp
                if idx == 0:
                    seg_feats_out.append(seg_temp)
                idx = idx + 1
                img_feats_out.append(filter_feat)
        # -------------提升的过程写在此处--------------------
        if self.lift:
            ###判断batch一致
    ...
    def forward_train(self,
        ...
        if pts_feats:
            ...
            if self.segmentation:
                ...
                losses_seg = self.seg_loss(seg_preds[0], seg_gt)
