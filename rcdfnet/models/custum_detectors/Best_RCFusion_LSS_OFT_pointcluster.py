...
class Best_RCFusion_LSS_OFT_pointcluster(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
        ...
        """
        #----------------pointcluster------------------------------------------------------------------
        pts_voxel_layer_cluster = kwargs.get('pts_voxel_layer_cluster', None)
        kwargs.pop('pts_voxel_layer_cluster')
        pts_cluster_spconv = kwargs.get('pts_cluster_spconv', None)
        kwargs.pop('pts_cluster_spconv')
        pts_cluster_spatial_shape = kwargs.get('spatial_shape', None)
        kwargs.pop('spatial_shape')
        pts_cluster_output_dim = kwargs.get('output_dim', None)
        kwargs.pop('output_dim')
        super(Best_RCFusion_LSS_OFT_pointcluster, self).__init__(**kwargs)
        ...
        self.spatial_shape = pts_cluster_spatial_shape
        self.reduce_channel1 = torch.nn.Conv2d(in_channels=int(pts_cluster_output_dim * pts_cluster_spatial_shape[2]),
                                               out_channels=int(pts_cluster_output_dim), kernel_size=1)
        self.reduce_channel2 = torch.nn.Conv2d(in_channels=int(pts_cluster_output_dim * pts_cluster_spatial_shape[2]),
                                               out_channels=int(pts_cluster_output_dim), kernel_size=1)
        ####-----------LSS分支初始化--------
        if camera_stream:
    ...
    def extract_pts_feat(self, pts, img_metas):
        ...
        voxels, num_points, coors = self.voxelize(pts, voxelize1)
        voxel_features = self.pts_voxel_encoder(voxels, num_points, coors)
        batch_size = coors[-1, 0].item() + 1
        x = self.pts_middle_encoder(voxel_features, coors, batch_size)
        x = self.pts_backbone(x)

        points_cluster_R1 = Density_extract(pts, R=1.5)
        points_cluster_R2 = Density_extract(pts, R=2)
        ...
        sparse_input_R1 = SparseConvTensor(voxels_cluster1.squeeze(dim=1), coors_cluster1.index_select(1, order),
                                           self.spatial_shape, batch_size)
        sparse_input_R2 = SparseConvTensor(voxels_cluster2.squeeze(dim=1), coors_cluster2.index_select(1, order),
                                           self.spatial_shape, batch_size)
