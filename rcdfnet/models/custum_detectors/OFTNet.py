...
class OftNet(nn.Module):  #（B,256,216,248）
    ...
    def forward(self, img_feats, calib):
        ...
        # feats4, feats8, feats16, feats32 = img_feats
        # feats4, feats8, feats16= img_feats
        # feats8 = img_feats
        ...
        # ortho = torch.cat([self.attention_block(ortho4), self.attention_block(ortho8), self.attention_block(ortho16)], dim=1)
        # ortho = self.attention_block(ortho)
...
class OftNet_EMCAD(nn.Module):  #（B,256,216,248）
    ...
    def __init__(self, img_bev_harf=True, grid_size=(69.12,79.36), grid_offset=(0,-39.68, 0), grid_res=0.32, grid_z_min=-3, grid_z_max=2.76):
        ...
        if img_bev_harf:
            self.bevencoder = nn.Sequential(
                nn.Conv2d(256 * 3, 256, kernel_size=3, stride=2, padding=1, bias=False),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
                nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(256),
            ...
            self.bevencoder = nn.Sequential(
                nn.Conv2d(256*3,256,kernel_size=3,stride=1,padding=1,bias=False),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
                nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(256),
        ...
        self.attention_block = Attention_Block(256)
        # self.emcad = EMCAD(channels=[256,256,256,256,256], kernel_sizes=[1, 3, 5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu')
        #---------------------------
    def forward(self, img_feats, calib):
        # Normalize by mean and std-dev
        ...
        ##################################EMCAD###############################################
        # ortho_emcad = self.emcad(ortho64, [ortho32, ortho16, ortho8, ortho4])
        # ortho = torch.cat(ortho_emcad, dim=1)
        #----------------------
        # ortho = torch.cat([self.attention_block(ortho4),self.attention_block(ortho8),self.attention_block(ortho16), self.attention_block(ortho32),self.attention_block(ortho64)],dim=1)
        # ortho = torch.cat([self.attention_block(ortho4), self.attention_block(ortho8), self.attention_block(ortho16), self.attention_block(ortho32)], dim=1)
        ortho = torch.cat([self.attention_block(ortho4), self.attention_block(ortho8), self.attention_block(ortho16)], dim=1)
        # ortho = self.attention_block(ortho)
        ortho = self.bevencoder(ortho)
...
class OftNet1(nn.Module):  #（B,256,216,248）
    ...
    def __init__(self, img_bev_harf=True, grid_size=(69.12,79.36), grid_offset=(0,-39.68, 0), grid_res=0.32, grid_z_min=-3, grid_z_max=2.76):
        ...
        self.oft32 = OFT(256, input_yxchannels, 1 / 32.)
        #-------------bev img encoder--------------------------
        # self.bev_encoder = Residual(256, 256, False)
        #-------------
        self.attention_block = Attention_Block(256)
        #---------------------------
    def forward(self, img_feats, calib):
        ...
        # Apply OFT and sum
        ortho32 = self.oft32(feats, calib, self.corners_radar)
        ortho = self.attention_block(ortho32)
