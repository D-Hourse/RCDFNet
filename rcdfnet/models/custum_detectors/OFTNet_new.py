...
class OFT(nn.Module):
    ...
    def forward(self, features, radar_att, calib, corners_radar):   ##corners_radar(1,18,216,248,3)
        ...
        batch, height, depth, width, _ = bbox_corners.size()  ##（B,7,159,159,4）
        ...
        radar_att = radar_att.view(batch, height, -1).unsqueeze(1)
        vox_feats = radar_att * vox_feats
        vox_feats = vox_feats.permute(0, 3, 1, 2).flatten(0, 1).flatten(1, 2)  # (b*25281,1792)
...
class OftNet(nn.Module):  #（B,256,216,248）
    ...
    def __init__(self, img_bev_harf=True, grid_size=(69.12,79.36), grid_offset=(0,-39.68, 0), grid_res=0.32, grid_z_min=-3, grid_z_max=2.76):
        ...
        #---------------------------
    def forward(self, img_feats, radar_att, calib):
        ...
        # Apply OFT and sum
        ortho4 = self.oft4(feats4, radar_att, calib, self.corners_radar)
        ortho8 = self.oft8(feats8, radar_att, calib, self.corners_radar)
        ortho16 = self.oft16(feats16, radar_att, calib, self.corners_radar)
        ...
        # ortho = self.attention_block(ortho8)
