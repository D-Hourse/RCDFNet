...
class Best_RCFusion_Centerhead(RC_backbone):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
        ...
        """
        super(Best_RCFusion_Centerhead, self).__init__(**kwargs)
        ...
        if rc_fusion == "concat":  ##融合层
            ...
            #     act_cfg=dict(type='ReLU'),
            #     inplace=False)
            self.cross_attention = Cross_Modal_Fusion_test(kernel_size=3)
    ...
    def forward_train(self,
        ...
        losses = dict()
        if pts_feats:
            losses_pts = self.forward_pts_train(bev_out, gt_bboxes_3d,
