...
class Best_RCFusion_triple_att(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True,
                 ...
                 downsample_factor=32, depth_net_in_channels=256, depth_net_mid_channels=256, return_depth=False, **kwargs):
        """
        Args:
        ...
        """
        super(Best_RCFusion_triple_att, self).__init__(**kwargs)
        ...
        if camera_stream:
            ...
            self.bev1_attention = BEVFuser(bev1_dims=256, bev2_dims=384, embed_dims=256, num_layers=3, num_heads=4, bev_shape=(160, 160))
            self.bev2_attention = BEVFuser(bev1_dims=256, bev2_dims=384, embed_dims=256, num_layers=3, num_heads=4, bev_shape=(160, 160))
        if rc_fusion == "concat":  ##融合层
            ...
            self.cross_attention = BEVFuser(bev1_dims=256, bev2_dims=256, embed_dims=256, num_layers=6, num_heads=4, bev_shape=(160, 160))
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if self.lift:
            ...
            if self.return_depth:
                img_bev_feat_L_scale = self.oft_BEVencoder(img_feats[:-1], calib)
            ...
            if pts_feats is None:
                pts_feats = self.cross_attention(img_bev_feat_L_scale, img_bev_feat_S_scale)  ####cam stream only
            else:
                # --------融合成为pts_feats----------------
                pts_feats1 = self.bev1_attention(img_bev_feat_L_scale,  pts_feats[0])
                pts_feats2 = self.bev2_attention(img_bev_feat_S_scale,  pts_feats[0])
                pts_feats = [self.cross_attention(pts_feats1, pts_feats2)]
    ...
    def forward_train(self,
        ...
        losses = dict()
        if pts_feats:
            losses_pts = self.forward_pts_train(pts_feats, depth_preds, depth_gt, gt_bboxes_3d,
