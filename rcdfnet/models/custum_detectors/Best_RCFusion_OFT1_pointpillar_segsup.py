...
from .OFTNet import OftNet1
from .LSSNet import LSSNet, IFSO
from ..module.EMCAD import CSIE
...
class Best_RCFusion_OFT1_segsup_Focolloss(MVXFasterRCNN):
    ...
    def __init__(self,image_size=None, img_bev_harf=False, rc_fusion="concat", camera_stream=True, segmentation=False,
        ...
        """
        img_decoder = kwargs.get('img_decoder', None)
        kwargs.pop('img_decoder')
        img_feature_seg_out = kwargs.get('IFSO', None)
        kwargs.pop('IFSO')
        freeze_radar = kwargs.get('freeze_radar', None)
        kwargs.pop('freeze_radar')
        super(Best_RCFusion_OFT1_segsup_Focolloss, self).__init__(**kwargs)
        ...
        if self.segmentation:
            self.img_decoder = CSIE(**img_decoder)
            self.IFSO = IFSO(**img_feature_seg_out)
            self.seg_loss = FocalLoss()

        ####-----------LSS分支初始化--------
        if camera_stream:
            self.oft_BEVencoder = OftNet1(img_bev_harf=img_bev_harf, grid_size=grid_size, grid_offset=grid_offset,
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        # -------------segmentation-----------------------
        if self.segmentation:
            # img_feat = self.img_decoder(img_feats)
            feats_temp = self.IFSO(img_feats[-1], img_metas)
            seg_temp = feats_temp[1].softmax(dim=1)
            img_mask = torch.sum(seg_temp[:, 1:4, :, :], dim=1) >= self.seg_threshold
            filter_feat = img_mask.unsqueeze(dim=1) * feats_temp[0]
            seg_feats_out = [seg_temp, img_mask]
        ...
        if self.lift:
            ...
            if self.return_depth:
                img_bev_feat_L_scale = self.oft_BEVencoder(filter_feat, calib)
            ...
            img_bev_feat = img_bev_feat_L_scale
        ...
        if self.segmentation:
            return_dict.update(seg_preds=seg_feats_out)
        return return_dict
    ...
    #         seg[seg > 0] = 1
    #         H, W = seg.shape
    #         seg = seg.view(H//4, 4, W//4, 4).contiguous()
    #         seg = seg.view(-1, 4*4)
    #         seg = torch.max(seg, dim=-1).values
    #         seg = seg.view(H//4, W//4).contiguous()
    ...
    def seg_gt_transform(self, segmentation):
        ...
        for seg in segmentation:
            seg[seg > 0] = 1
            H, W = seg.shape
            seg = seg.view(H//32, 32, W//32, 32).contiguous()
            seg = seg.view(-1, 32*32)
            seg = torch.max(seg, dim=-1).values
            seg = seg.view(H//32, W//32).contiguous()
            seg_gt_list.append(seg.unsqueeze(dim=0))
        segmentation = torch.cat(seg_gt_list, dim=0)
        # one_hot = torch.zeros(segmentation.size(0), self.num_classes+1, segmentation.size(1), segmentation.size(2), dtype=torch.float32).to(segmentation.device)
        # one_hot.scatter_(1, segmentation.unsqueeze(1), 1)
    ...
    def forward_train(self,
        ...
        if pts_feats:
            ...
            if self.segmentation:
                ...
                losses_seg = self.seg_loss(seg_preds[0], seg_gt, seg_preds[1])
    ...
    def seg_loss_bce(self, seg_preds, seg_gt):
        ...
        for seg_pred in seg_preds:
            ...
            loss_seg = loss_seg + loss_seg_temp/num_feat
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, seg_mask):

        iou = compute_iou(seg_mask, targets)
        # print("now iou:", iou)
        # 确保输入是概率
        ...
        if self.reduction == 'mean':
            return dict(loss_seg=[focal_loss.sum() / (inputs.size(0)*inputs.size(1)*inputs.size(2))])
        elif self.reduction == 'sum':
            return dict(loss_seg=[focal_loss.sum()])
        else:
            return dict(loss_seg=[focal_loss])


def compute_iou(ground_truth_mask, prediction_mask):
    """
    计算IoU（Intersection over Union）使用PyTorch

    参数：
    ground_truth_mask (torch.Tensor): 真实前景区域的二进制掩码，形状为 (H, W)
    prediction_mask (torch.Tensor): 预测前景区域的二进制掩码，形状为 (H, W)
    ...
    # 计算交集
    intersection = torch.logical_and(ground_truth_mask, prediction_mask).sum().item()

    # 计算并集
    union = torch.logical_or(ground_truth_mask, prediction_mask).sum().item()
