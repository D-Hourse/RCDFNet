...
from rcdfnet.core import (PseudoSampler, box3d_multiclass_nms, limit_period,
                          xywhr2xyxyr)
...
class My_Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    Args:
        ...
        loss_depth (dict): Config of depth loss.
    ...
    def __init__(self,
                 ...
                 # loss_depth=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),
                 loss_depth=None,
                 init_cfg=None):
        ...
        self.loss_depth = loss_depth['use_depth']
        self.depth_weight = loss_depth['loss_weight']
        self.fp16_enabled = False
    ...
    def loss_single(self, cls_score, bbox_pred, dir_cls_preds, depth_preds, depth_gt, labels,
        ...
        Args:
            ...
            dir_cls_preds (torch.Tensor): Predictions of direction class
                in single-level.
            depth_preds (torch.Tensor): Prediction of the depth distribution
                features map.
            depth_gt (torch.Tensor): GT of the depth distribution features map.
        ...
        Returns:
            tuple[torch.Tensor]: Losses of class, bbox \
                 ,direction and depth, respectively.
        """
        # classification loss
        ...
        # loss_depth = self.loss_depth(depth_preds, depth_gt)
        if self.loss_depth:
            loss_depth = (F.binary_cross_entropy(depth_preds[fg_mask], depth_gt[fg_mask], reduction='none',).sum() / max(1.0, fg_mask.sum())) * self.depth_weight
        # dir loss
        if self.use_direction_classifier:
        ...
        return loss_cls, loss_bbox, loss_dir, loss_depth
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox, losses_dir, losses_depth = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            dir_cls_preds,
        ...
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_dir=losses_dir, loss_depth=losses_depth)
