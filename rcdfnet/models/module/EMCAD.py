...
class EMCAD(nn.Module):
    def __init__(self, channels=[256, 256, 256, 256, 256], kernel_sizes=[1, 3, 5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu'):
...
class CSIE(nn.Module):
    def __init__(self,  channels=256):
        ...
        self.up1 = Up_Module(in_channels=channels, out_channels=channels)
        self.up2 = Up_Module(in_channels=channels, out_channels=channels)
        self.up3 = Up_Module(in_channels=channels, out_channels=channels)

        self.cab4 = CAB(channels)
        self.cab3 = CAB(channels)
        self.cab2 = CAB(channels)
        self.cab1 = CAB(channels)
    ...
    def forward(self, x):
        ...
        d3 = self.sab3(x3) * x3
        d3 = self.cab3(d3) * d3
        d3 = self.norm_act(d4 + d3)
        d3 = self.up2(d3)

        d2 = self.sab3(x2) * x2
        d2 = self.cab3(d2) * d2
        d3 = self.norm_act(d3 + d2)
        d2 = self.up3(d2)

        d1 = self.sab3(x1) * x1
        d1 = self.cab3(d1) * d1
        d1 = self.norm_act(d2 + d1)
...
class EMCAD_FPN(nn.Module):
    def __init__(self, channels=[2048, 1024, 512, 256], out_channels=256, kernel_sizes=[1, 3, 5], expansion_factor=6, dw_parallel=True,
                 add=True, lgag_ks=3, activation='relu'):
        super(EMCAD_FPN, self).__init__()
        eucb_ks = 3  # kernel size for eucb
        ...
        self.reduce1 = nn.Conv2d(channels[3], out_channels, 1)
        self.reduce2 = nn.Conv2d(channels[2], out_channels, 1)
        self.reduce3 = nn.Conv2d(channels[1], out_channels, 1)
        self.reduce4 = nn.Conv2d(channels[0], out_channels, 1)
    ...
    def forward(self, x):
        ...
        return [self.reduce1(d1), self.reduce2(d2), self.reduce3(d3), self.reduce4(d4)]