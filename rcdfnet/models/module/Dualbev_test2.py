...
from ..custum_detectors.LSSNet import DepthNet
...
class HeightTransformer:
    def __init__(self, pc_range, bev_h=128, bev_w=128, num_height=13, collapse_z=True,
                 depth_threshold=1, semantic_threshold=0.25, depthnet_cfg=dict(), **kwargs):
        super(HeightTransformer, self).__init__(**kwargs)
    ...
    def pre_compute(self, input):
        if self.initial_flag:
            ...
            coor, mask = self.get_sampling_point(voxel, self.pc_range, self.grid_config['depth'], self.lidar2img,
                                                 self.img_aug, self.input_size)
    ...
    def get_ht_bev_feat(self, input, depth, tran_feat, bev_mask=None):
        ...
        if self.accelerate:
            ...
            coor, mask = self.get_sampling_point(voxel, self.pc_range, self.grid_config['depth'], lidar2img, img_aug,
                                                 self.input_size)
