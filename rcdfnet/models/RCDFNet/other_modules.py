...
# from ..cpf_detectors.mvx_two_stage import MVXTwoStageDetector
    ...
    def img_point_sampling(self, reference_voxel, mlvl_feats, batch_size=4, **kwargs):  # from UVTR
        ...
        for b in range(batch_size):
            ...
            if img_aug_matrix is not None:
                ...
                if not torch.is_tensor(cur_img_aug_matrix):
                    cur_img_aug_matrix = torch.as_tensor(cur_img_aug_matrix, device=cur_coords.device)
                else:
                    cur_img_aug_matrix = cur_img_aug_matrix.to(cur_coords.device)
                if cur_img_aug_matrix.dim() == 3:
                    cur_img_aug_matrix = cur_img_aug_matrix[0]
            if lidar_aug_matrix is not None:
                ...
                if not torch.is_tensor(cur_lidar_aug_matrix):
                    cur_lidar_aug_matrix = torch.as_tensor(cur_lidar_aug_matrix, device=cur_coords.device)
                else:
                    cur_lidar_aug_matrix = cur_lidar_aug_matrix.to(cur_coords.device)
                if cur_lidar_aug_matrix.dim() == 3:
                    cur_lidar_aug_matrix = cur_lidar_aug_matrix[0]
            ...
            # lidar2image
            if not torch.is_tensor(cur_lidar2image):
                cur_lidar2image = torch.as_tensor(cur_lidar2image, device=cur_coords.device)
            else:
                cur_lidar2image = cur_lidar2image.to(cur_coords.device)
...
class PointFusion_att(BaseModule):
    ...
    Args:
        ...
        coder_norm_cfg (dict): Config of last normalization layer. Default：
            `LN`.
    """
    def __init__(self, num_points_in_pillar=10, embed_dims_img=256, embed_dims_radar=384, bev_size=256, scale_factor=None, region_shape=None, grid_size=None, region_drop_info=None, return_proj_info=False):
        super(PointFusion_att, self).__init__()
        self.return_proj_info = return_proj_info
        ...
        self.bev_pos = create_2D_grid(self.bev_size[0], self.bev_size[1])
    ...
    def img_fv_to_bev(self, mlvl_feats, bs, **kwargs):
        ...
        decorated_img_feat = torch.zeros([bs, self.embed_dims_img, self.bev_size[0], self.bev_size[1]]).type_as(mlvl_feats[0])
        for b in range(bs):
            this_pillar_coors = pillar_coors_list[b]
            output = ref_img_pillar_cam[0][b].reshape(self.embed_dims_img, -1, num_points_in_pillar)
            output = output.sum(dim=2)
        ...
        proj_info = dict(
            reference_voxel_cam=reference_points_cam_stack,
            mask=ref_mask_stack,
            pillar_coors_list=pillar_coors_list,
        )
        return decorated_img_feat, proj_info
    ...
    def forward(self,
                ...
                bs,
                **kwargs):
        img_bev_feats, proj_info = self.img_fv_to_bev([img_mlvl_feats], bs, **kwargs)
        ...
        if self.return_proj_info:
            return bev_feats, proj_info
        return bev_feats
