...
from rcdfnet.core.bbox import BaseInstance3DBoxes
from rcdfnet.core.points import BasePoints
...
class DefaultFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        if 'depth' in results:
            assert isinstance(results['depth'], BasePoints)
            results['depth'] = DC(results['depth'].tensor)

        if 'seg_label' in results:
            results['seg_label'] = DC(to_tensor(results['seg_label']))
