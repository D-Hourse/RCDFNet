...
from ..core import show_multi_modality_result, show_multi_modality_result_my, show_result
...
class VoDDataset_Seg(Custom3DDataset):
    r"""KITTI Dataset.
    ...
    def _get_depth_filename(self, idx):
        ...
        """
        depth_filename = osp.join(self.root_split, self.depth_prefix,
                                f'{idx:05d}.jpg.bin')
        return depth_filename

    def _get_seg_filename(self, idx):
        """Get image segmentation filename according to the given index.

        Args:
            index (int): Index of the image file to get.

        Returns:
            str: Name of the image segmentation file.
        """
        seg_filename = osp.join(self.root_split, self.seg_prefix,
                                f'{idx:05d}.jpg.npy')
        return seg_filename
    ...
    def get_data_info(self, index):
        ...
        depth_filename = self._get_depth_filename(sample_idx)
        seg_filename = self._get_seg_filename(sample_idx)
        input_dict = dict(
    ...
    def evaluate(self,
        ...
        from rcdfnet.core.evaluation import kitti_eval
        ...
        if tmp_dir is not None:
            tmp_dir.cleanup()
        if show:
            self.show_my(results, out_dir, pipeline=pipeline)
        return ap_dict
    ...
    def show(self, results, out_dir, show=True, pipeline=None):
        ...
        for i, result in enumerate(results):
            ...
            points, img_metas, imgs = self._extract_data(
    ...
    def show_my(self, results, out_dir, show=False, pipeline=None):
        """Results visualization.
