...
def get_velodyne_path(idx,
                      ...
                      use_prefix_id=False,
                      file_tail='.jpg'):
    return get_kitti_info_path(idx, prefix, 'velodyne', file_tail, training,
                               relative_path, exist_check, use_prefix_id)
...
def get_calib_path(idx,
                   ...
                   use_prefix_id=False,
                   file_tail='.jpg'):
    return get_kitti_info_path(idx, prefix, 'calib', file_tail, training,
                               relative_path, exist_check, use_prefix_id)
...
def get_pose_path(idx,
                  ...
                  use_prefix_id=False,
                  file_tail='.jpg'):
    return get_kitti_info_path(idx, prefix, 'pose', file_tail, training,
                               relative_path, exist_check, use_prefix_id)
...
def get_kitti_image_info(path,
    ...
    def map_func(idx):
        ...
        ####################################
        if depth:
            info['lidar_depth'] = dict(
                num_features=3,
                image_idx=info['image']['image_idx'],
                depth_path = os.path.join('training/depth_gt', os.path.basename(info['image']['image_path']) + '.bin')
            )
        else:
            info['lidar_depth'] = None
        ####################################
        if calib:
            ...
            # Tr_imu_to_velo = np.zeros((3,4))  # vod
            if extend_matrix:
    ...
    # image_infos = map_func(image_ids)
...
def add_difficulty_to_annos(info):  # 根据图像内目标的高度和遮挡截断综合考虑
    min_height = [40, 25,
